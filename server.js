...
// এই সার্ভারের কাজ: OpenAI (ChatGPT-এর ইমেজ মডেল) API key নিরাপদে রাখা এবং
// ফ্রন্টএন্ডের হয়ে ব্যাকগ্রাউন্ড-রিমুভাল রিকোয়েস্ট পাঠানো।
...
if (!OPENAI_API_KEY) {
  console.error('⚠️  OPENAI_API_KEY সেট করা নেই! .env ফাইলে OPENAI_API_KEY=আপনার_কী বসান।');
}

const openai = new OpenAI({ apiKey: OPENAI_API_KEY });
// মডেল বদলাতে চাইলে .env-এ OPENAI_IMAGE_MODEL বসিয়ে দিন — কোড বদলাতে হবে না।
// gpt-image-2 = সেরা কোয়ালিটি (ডিফল্ট), gpt-image-1-mini = সবচেয়ে কম খরচ
...
const BG_INSTRUCTIONS = {
  white: 'Replace the background with a solid, seamless, pure white (#FFFFFF) backdrop, exactly like a professional passport photo studio.',
  blue: 'Replace the background with a solid, seamless, professional light blue (#4A6FA5) backdrop, exactly like a passport photo studio.',
  transparent: 'Remove the background completely and output a PNG with a fully transparent background, leaving only the subject.'
};
...
app.post('/api/remove-background', upload.single('image'), async (req, res) => {
  try {
    ...
    if (!OPENAI_API_KEY) {
      return res.status(500).json({ error: 'সার্ভারে OPENAI_API_KEY সেট করা নেই' });
    }

    const bgChoice = req.body.background;
    const bgInstruction = BG_INSTRUCTIONS[bgChoice] || BG_INSTRUCTIONS.white;

    const prompt = `Edit this portrait photo for an official passport/ID photo studio. ` +
      `Carefully preserve the person exactly as they are — same face, expression, pose, clothing, ` +
      `and lighting on the subject — with fine details like hair strands and clothing edges kept sharp and intact. ` +
      `${bgInstruction} The result must look photorealistic, not like an illustration or painting.`;

    // gpt-image-2 (ডিফল্ট) সবচেয়ে ভালো কোয়ালিটি দেয়, কিন্তু সত্যিকারের transparent
    // ব্যাকগ্রাউন্ড আউটপুট করতে পারে না — সেক্ষেত্রে gpt-image-1.5 ব্যবহার হবে।
    const modelForThisRequest = (bgChoice === 'transparent' && IMAGE_MODEL === 'gpt-image-2')
      ? 'gpt-image-1.5'
      : IMAGE_MODEL;

    const imageFile = await toFile(req.file.buffer, 'photo.png', {
    ...
    if (!b64) {
      return res.status(502).json({ error: 'ChatGPT থেকে কোনো ছবি ফেরত আসেনি, আবার চেষ্টা করুন' });
    }
